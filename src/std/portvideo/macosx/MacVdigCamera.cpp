...
bool MacVdigCamera::findCamera() {
  ...
  if(!(pVdg = vdgNew()))    {
    fprintf(stderr, "vdgNew err\n");
      return false;
  }
  if(err = vdgInit(pVdg))    {
    fprintf(stderr, "no camera found - vdgInit err=%d\n", err);
    return false;
  }
  cameraID = 0;
  return true;
...
bool MacVdigCamera::initCamera() {
  ...
  IF_DEBUG(fprintf(stderr, "INIT CAMERA\n", config.device););	
  OSErr err;
  IF_DEBUG(fprintf(stderr, "config.device %d\n", config.device););	
  if (config.device!=SETTING_AUTO) {
    if(err = vdgRequestSettingsNoGUI(pVdg))
      {
	fprintf(stderr, "camera setup cancelled - vdgRequestSettings err=%d\n", err);
	return false;
      }
  } else {
  ...
  if(err = vdgPreflightGrabbing(pVdg)) {
      fprintf(stderr, "vdgPreflightGrabbing err=%d\n", err);
      return false;
  }
  vdImageDesc = (ImageDescriptionHandle) NewHandle(0);
  if (err = vdgGetImageDescription( pVdg, vdImageDesc))  {
  ...
  if (err = createOffscreenGWorld(	&dstPort,
					...
					&dstPortBounds))
    {
      fprintf(stderr, "createOffscreenGWorld err=%d\n", err);
      return false;	
    }
  ...
  if (err = vdgSetDestination(	pVdg, dstPort )) {
      fprintf(stderr, "vdgSetDestination err=%d\n", err);
      return false;
  }
  ...
  IF_DEBUG(fprintf(stderr, "VDIG buffer [%s] %lx (%d %d, %d) %d\n", cameraName, buffer, this->width, this->height, bytes, this->width*this->height*bytes););
  return true;
}
...
unsigned char* MacVdigCamera::getFrame()
{
  ...
  if (err = vdgIdle( pVdg, &isUpdated))
    {
      fprintf(stderr, "could not grab frame\n");
      return NULL;
    }
  if (isUpdated)
    {
...
bool MacVdigCamera::startCamera()
{
  ...
  if (err = vdgStartGrabbing(pVdg))
    {
      fprintf(stderr, "could not start camera\n");
      return false;
    }
...
bool MacVdigCamera::stopCamera()
{
  ...
  if (err = vdgStopGrabbing(pVdg))
    {
      fprintf(stderr, "errors while stopping camera\n");
      return false;
    }
  return true;
}
