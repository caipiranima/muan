...
bool DSlibCamera::initCamera() {
  ...
  if (w>0) this->width = config.width = w;
  if (h>0) this->height = config.height = h;
  if (f>0) this->fps = config.fps = f;
...
unsigned char* DSlibCamera::getFrame()
{
  ...
  if(wait_result == WAIT_OBJECT_0)   {
    ...
    case true: {
      unsigned char *src = (unsigned char*)buffer;
      unsigned char *dest = pbuffer;
      unsigned int block_size = width*bytes;

      bgr_to_rgb(src, width, height);

      dest += width*height*bytes;
